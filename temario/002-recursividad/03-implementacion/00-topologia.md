...
La pila de llamadas no es un artefacto de la recursión. Existe en cada llamada a función, recursiva o no:
...
Cada función tiene su propia variable `spam`. Las variables no se mezclan porque cada llamada crea su propio **marco** en la pila con sus propias variables locales. Los marcos se apilan y desapilan en orden LIFO.

Con eso claro, las siguientes fases exploran qué ocurre cuando esa pila crece recursivamente.
...
¿Qué imprimirá `misterio(3)` y en qué orden?
